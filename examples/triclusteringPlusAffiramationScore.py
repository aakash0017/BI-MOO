...
def mean_squared_residue_np(matrix, rows, cols, inverted_rows=np.array([])):
    ...
    if len(rows) == 0 or len(cols) == 0:
        ...
        def msr(a):
            return (np.power(a - a.mean(axis=1, keepdims=True) - a.mean(axis=0) + a.mean(), 2).mean())
...
def hide_tricluster_np(matrix, rows, cols, inverted_rows=np.array([])):
    """
...
def plot_tricluster(matrix, bicluster, name="Bicluster"):
    """
    Plot a bicluster.
    Parameters
    ----------
...
def find_triclusters_np(matrix, n_of_bicluster=100, msr_threshold=20, alpha=1.2):
    """
    ...
    for i in range(n_of_bicluster):
        ...
        biclusters.append(Tricluster(rowsD, colsD, invD, msr))
        matrixA = hide_tricluster_np(matrixA, rowsD, colsD, invD)
        print(len(rowsD))
        print(len(colsD))
...
def setTriclustersMatchScore(setTriclusters1, setTriclusters2):
    ...
    return math.sqrt(rowsScore * colsScore * timesScore)


def main():
    path1 = ''
    names1 = [path1 + 'tp1', path1 + 'tp2', path1 + 'tp3', path1 + 'tp4']
    data1 = readFiles(names1)
    # data = read_matrix("http://arep.med.harvard.edu/biclustering/lymphoma.matrix",url=True)
    # data = clean(data)

    rows = np.arange(0, data1.shape[0])
    cols = np.arange(0, data1.shape[1])
    times = np.arange(0, data1.shape[2])
    msr = mean_squared_residue_np(data1, rows, cols, times)
    print("MSR before multiple_deletion_node\t" + str(msr))

    ### mean_squared_residue_np & init done ###

    start = time.time()
    triclusters1 = find_triclusters_np(data1)
    end = (time.time() - start)
    print(end, "seconds")
    best_tricluster = sorted(triclusters1, key=lambda tricluster: tricluster.msr)[1]
    print(best_tricluster)
    # plot_tricluster(data, best_tricluster)

    ################################################

    '''
    path2 = 'datatoupload/GSE11324/'
    names2 =[path2+'tp1',path2+'tp2', path2+'tp3',path2+'tp4']
    data2 = readFiles(names2)
    #data = read_matrix("http://arep.med.harvard.edu/biclustering/lymphoma.matrix",url=True)
    #data = clean(data)

    rows = np.arange(0, data2.shape[0])
    cols = np.arange(0, data2.shape[1])
    times = np.arange(0, data2.shape[2])
    msr = mean_squared_residue_np(data2, rows, cols, times)
    print("MSR before multiple_deletion_node\t" + str(msr))
    ...
    start = time.time()
    triclusters2 = find_triclusters_np(data2)
    end = (time.time() - start)
    print(end, "seconds")
    best_tricluster = sorted(triclusters2, key=lambda tricluster: tricluster.msr)[1]
    print(best_tricluster)
    #plot_tricluster(data, best_tricluster)
...
# print(setTriclustersMatchScore(triclusters1, triclusters2))
